...
## Loading the data set, exploring, and cleaning
Before we start doing association rules mining, we need to load the required data set - `bankdata_csv_all.csv` -, explore it, and clean it.

```{r tidy=TRUE}
dat <- read.csv('bankdata_csv_all.csv', stringsAsFactors = FALSE)
...
# Let's see the top 10 rules that we have found. 
options(digits = 3)
...
## PEP analysis
Now, let's explore what rules can explain whether a customer takes a PEP or not. 
```{r tidy=TRUE}
